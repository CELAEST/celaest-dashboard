...
export const ProductCardPremium: React.FC<ProductCardPremiumProps> = ({
  ...
  return (
    <motion.div
      ...
      `}
      style={{
        boxShadow: isHovered && theme === 'dark' 
    ...
    >
      ...
      <div className="relative h-[280px] w-full overflow-hidden">
        ...
        >
          <ImageWithFallback
            src={image}
            className={`w-full h-full object-cover ${theme === 'dark' ? 'opacity-90' : 'opacity-100'}`}
            alt={title}
          />
        </motion.div>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="space-y-2 pt-2">
          {features.map((feature, index) => (
            <div key={index} className="flex items-start gap-2">
              <Check 
                size={16} 
                className={`mt-0.5 flex-shrink-0 ${theme === 'dark' ? 'text-cyan-400' : 'text-cyan-600'}`} 
              />
        ...
        <div className="pt-4 border-t border-white/10">
          <div className="flex items-end justify-between mb-4">
            <div>
              <div className={`text-xs uppercase tracking-wider mb-1 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>
                Inversión
              </div>
