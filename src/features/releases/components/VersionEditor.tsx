...
export const VersionEditor: React.FC<VersionEditorProps> = ({ isOpen, onClose, onSave, version }) => {
  ...
  useEffect(() => {
    if (version) {
      setFormData({
        ...
        changelog: version.changelog.length > 0 ? version.changelog : [''],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanChangelog = formData.changelog.filter(item => item.trim() !== '');
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-sm mt-1 ${
                  ...
                  {version ? 'Update changelog and release metadata' : 'Publish a new version with full changelog'}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                  ...
                  <option value="deprecated">Deprecated (Legacy support only)</option>
                </select>
              </div>
                ...
                {formData.changelog.map((item, index) => (
                  <div key={index} className="flex gap-3">
                    <input
                      type="text"
                      value={item}
                      onChange={(e) => handleChangelogChange(index, e.target.value)}
            ...
            <div className={`p-4 rounded-xl border flex gap-3 ${
              ...
              <div>
                ...
                <p className={`text-xs ${
                  ...
                  Customers can verify file integrity using the SHA-256 checksum. This ensures downloads are not corrupted or tampered with.
                </p>
              </div>
            </div>
