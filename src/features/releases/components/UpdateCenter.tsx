        ...
        <div className="flex items-start justify-between">
          <div>
            <h2 className={`text-2xl font-bold mb-2 ${
              ...
              {assets.filter(a => a.hasUpdate).length} Update{assets.filter(a => a.hasUpdate).length !== 1 ? 's' : ''} Available
            </h2>
            <p className={`text-sm ${
              isDark ? 'text-gray-400' : 'text-gray-600'
            }`}>
              Stay current with the latest features, security patches, and performance improvements
            </p>
          </div>
          <Sparkles size={32} className={isDark ? 'text-cyan-400' : 'text-cyan-600'} />
        </div>
      </div>
        ...
        {assets.map((asset, index) => (
          ...
          >
            ...
            <div className={`p-6 ${
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`p-3 rounded-xl ${
                  ...
                  {asset.hasUpdate ? (
                    <AlertCircle size={24} className={isDark ? 'text-cyan-400' : 'text-cyan-600'} />
                  ) : (
                    <CheckCircle2 size={24} className={isDark ? 'text-emerald-400' : 'text-emerald-600'} />
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-6 text-xs">
                <div className="flex items-center gap-2">
                  <Clock size={14} className={isDark ? 'text-gray-600' : 'text-gray-400'} />
                  <span className={isDark ? 'text-gray-500' : 'text-gray-600'}>
                    Purchased: {new Date(asset.purchaseDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                  </span>
                </div>
              ...
              <div className={`border-t ${isDark ? 'border-white/5' : 'border-gray-200'}`}>
                ...
                {expandedAsset === asset.id && (
                  <motion.div
                    ...
                    className={`px-6 pb-6 border-t ${isDark ? 'border-white/5' : 'border-gray-200'}`}
                  >
            ...
            <div className={`p-6 border-t flex gap-3 ${
              ...
              {asset.hasUpdate ? (
                <>
                  <button className={`flex-1 py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                    isDark
                  ...
                  <button className={`px-6 py-3 rounded-xl font-semibold text-sm transition-all ${
                    isDark
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className={`text-xs ${
              ...
              You can download any version released after your purchase date. Active subscription holders have access to all historical versions.
            </p>
          </div>
        </div>
      </div>
