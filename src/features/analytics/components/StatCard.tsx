...
export const StatCard = React.memo(function StatCard({
  ...
  // Memoize class names that depend on isDark
  const containerClassName = useMemo(
    () =>
      `relative overflow-hidden backdrop-blur-xl border rounded-2xl p-6 group transition-colors duration-300 ${
        isDark
      ...
      <div className="relative flex justify-between items-start z-10 h-full">
        ...
        <div className="relative w-16 h-16 flex items-center justify-center">
          {hologramImage ? (
            ...
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 ring-1 ${
                isDark
