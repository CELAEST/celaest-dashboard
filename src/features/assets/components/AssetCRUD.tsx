...
export const AssetCRUD: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this asset? This will create a new archived version.')) {
      setAssets(assets.map(a => a.id === id ? { ...a, status: 'archived' as const } : a));
      setActiveMenu(null);
    }
  };
    ...
    if (editingAsset) {
      ...
      setAssets(assets.map(a => 
        a.id === editingAsset.id 
          ? { ...a, ...assetData, updatedAt: new Date().toISOString().split('T')[0] }
          : a
      ));
    } else {
      // Create new
              ...
              {assets.map((asset, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border uppercase ${
